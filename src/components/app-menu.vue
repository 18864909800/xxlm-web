...
<template>
    ...
    <ul id="side-menu" class="metismenu">
        <li
            ...
            :key="`item-${item.name}`"
            v-if="($store.state.slideBar.loadFlag && (item.manager == 0 || item.manager == 2)) ||
(!$store.state.slideBar.loadFlag && (item.manager == 1 || item.manager == 2))"
            class="side-nav-title side-nav-item"
        >
            <!--            {{$store.state.slideBar.loadFlag}}-->
          ...
          "
            ...
            <ul v-if="hasItems(item)" class="nav-second-level">
                <li
                    ...
                    v-if="($store.state.slideBar.loadFlag && (subitem.manager == 0 || subitem.manager == 2)) ||
(!$store.state.slideBar.loadFlag && (subitem.manager == 1 || subitem.manager == 2))"
                    :key="`sub-item-${subitem.name}`"
                    :class="{ 'side-nav-item': hasItems(subitem) }"
                >
                    <!--                    <span>{{subitem.manager}}</span>-->
                    <a
